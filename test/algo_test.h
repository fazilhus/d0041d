...
void test_subsequence_max_sum() {
    ...
    for (std::size_t i = 32; i <= 4096; i *= 2) {
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        // auto [sumdnc, _dnc1, _dnc2] = subarray_max_sum_c1(v, 0, 31);
        int sumdnc = maxSubArraySum(v, 0, v.size() - 1);
        if (sumk != sumdnc) {
    ...
    //     std::cout << "Count: " << v.size() << ", Result: " << sum << ", Took " << dur << "ms\n";
    // }

    // std::cout << "\nDivide and Conquer\n";
    // for (const auto& v : input) {
    //     auto start = std::chrono::high_resolution_clock::now();
    //     auto [sum, _1, _2] = subarray_max_sum_c1(v, 0, v.size() - 1);
    //     auto dur = (double)std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now() - start).count() / 1000000;
    //     std::cout << "Count: " << v.size() << ", Result: " << sum << ", Took " << dur << "ms\n";
    // }
...
void test_pairs() {
    std::vector<std::vector<int>> input{};
    input.push_back({3, 2, 1, 4});
    for (std::size_t i = 4; i <= 16; i *= 2) {
        input.push_back(rfilled_vector(8));
        std::cout << "V: " << input.back() << '\n';
    }

    for (auto& v : input) {
        auto res = mergesort_and_count(v);
        std::cout << "V: " << v << "\nPairs: " << res << '\n';
    }
}
